...
class Rel: ObservableObject {
    @Published var mediaFeed: RelFeed?
    ...
    func getResponse() {
        ...
        let request = NSMutableURLRequest(url: NSURL(string: "https://api.themoviedb.org/3/movie/now_playing?language=en-US&page=2")! as URL,
        ...
        let dataTask = session.dataTask(with: request as URLRequest, completionHandler: { (data, response, error) -> Void in
            if let safeData = data {
                ...
                if let decodedData = try? decoder.decode(RelFeed.self, from: safeData) {
